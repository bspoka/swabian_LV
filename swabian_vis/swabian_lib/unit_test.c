...
{
    char *raw_filepath = argv[1];
    char output_filepath[] = "testing.photons";
    int sync_channel = atoi(argv[2]);
    uint64_t sync_period_ps = atoi(argv[3]);

    //swabian_convert_rawdata_to_photons(raw_filepath, output_filepath, sync_channel, sync_period_ps);

    uint64_t buffer[] = {2, 10, 1, 15, 2, 20, 1, 40, 2, 50};
    uint64_t *missing_sync_data = NULL;
    uint64_t first_sync = 0;
    uint64_t *first_sync_stamp = &first_sync;
    uint64_t *output_buffer = NULL;
    int out_length = 0;
    int *output_length = &out_length;
    int missing = 0;
    int *missing_events = &missing;

    swabian_buffer_to_photons(sync_channel, sync_period_ps, buffer, 10, 1, &missing_sync_data, missing_events,
         first_sync_stamp, &output_buffer, output_length);

    print_array(output_buffer, *output_length, "passed_buffer:");
    print_array(missing_sync_data, *missing_events, "passed_buffer:");


    /*
    char data_comment[] = "blahblah";
    int32_t acq_mode = 2;
    int32_t acq_time = 100;
    int32_t sync_rate = 4E6;
    uint64_t num_records = 298746;
    char data_path[] = "testing.ht3";
    write_swabian_ht3v2_header(data_comment, acq_mode, acq_time,
                                sync_rate, num_records, data_path);
    */
